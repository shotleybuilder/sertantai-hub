<script lang="ts">
	...
	type View = 'loading' | 'display' | 'edit';
	...
	<div class="bg-white rounded-lg shadow-lg p-8 space-y-6">
		...
		<dl class="space-y-3">
			...
			<div class="flex justify-between">
				...
				<dd class="text-sm font-mono text-gray-600">{org?.slug || '—'}</dd>
			</div>
			<div class="flex justify-between">
			...
			{#if $authStore.organizationId}
				<div class="flex justify-between">
					...
					<dd class="text-sm font-mono text-gray-400">{$authStore.organizationId}</dd>
				</div>
			{/if}
		</dl>
	...
	<div class="bg-white rounded-lg shadow-lg p-8 space-y-6">
		...
		<div class="flex gap-3">
			<button
				on:click={handleSave}
				disabled={loading}
				class="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors duration-200"
			>
				{loading ? 'Saving...' : 'Save Changes'}
			</button>
			<button
				on:click={cancel}
				class="px-4 py-2 text-gray-600 text-sm font-medium rounded-lg hover:bg-gray-100 transition-colors duration-200"
			>
				Cancel
			</button>
		</div>
